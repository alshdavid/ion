...
impl JsString {
    ...
    pub fn to_string_lossy(&self, env: &Env) -> crate::Result<String> {
...
impl<S: AsRef<str>> IntoJsValue<S> for JsString {
    fn into_js_value(env: &Env, value: S) -> crate::Result<Self> {
        let Some(value) = v8::String::new(env.scope(), value.as_ref()) else {
...
impl FromJsValue<String> for JsString {
    fn from_js_value(&self, env: &Env) -> crate::Result<String> {
        Ok(self.inner().to_rust_string_lossy(env.scope()))
    }
}

impl Env {
    pub fn create_string(&self, value: impl AsRef<str>) -> crate::Result<JsString> {
