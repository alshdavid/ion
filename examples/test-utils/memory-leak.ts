...
import { assertLessOrEqual } from "jsr:@std/assert@^1/less-or-equal";
...
export async function executeMemoryTest(
    testName: string,
    args: string[] = [],
    ...
    if (samples.length < 15) {
        throw new Error(
            "Not enough samples collected for reliable leak detection (need at least 15)",
        );
    }
    ...
    const lastQuarter = steadyStateSamples.slice(
        -Math.floor(steadyStateSamples.length / 4),
    );
...
export function assertMemoryLastQuarterGrowth(
